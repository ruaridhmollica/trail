...
func main() {
	...
	router.POST("/geofence/:lat/:long/:visited", func(c *gin.Context) {
		...
		rows, err := db.Query("SELECT id, treename, latinname, height, age, description, origin, img FROM trees WHERE ST_DWithin ( geography (ST_Point(longitude,latitude)), geography (ST_Point($1, $2)), 15) AND id != $3 limit 1", long, lat, visited)
		if err != nil {
			c.String(http.StatusInternalServerError,
				fmt.Sprintf("Error reading trees: %q", err))
			return
